function [AFTForTasks,ScheduleResult,relPerTask] = MSLFuc( DAG, relReq, failureRate)
%%
...
for i = 1:numProcessor
    eval(strcat('arrayTest = schedule.processor',num2str(i),';'));
    if size(arrayTest, 1) > 0
        [value, index] = sort(arrayTest(:,1), 'ascend');
        arrayTestResult = arrayTest(index, :);
        eval(strcat('schedule.processor',num2str(i),'= arrayTestResult;'));
    end
end
