function [DAG_DVFS, totalEnergyConsumption, ScheduleResultProi, frequencyScale1, ProscheduleLength] = DVFS_ProcessorFuc( DAG, relReq, failureRate, energySpec, d, MSLscheduleLength)

numProcessor = size(DAG.Wcet, 1); %下面几行到空行之前是初始化
numTask = size(DAG.E, 1);
frequencyScale(1:numProcessor) = 1;
frequencyScale1 = frequencyScale;
DAG_backup = DAG;
DAG_backup1 = DAG;
failureRate_backup = failureRate;
% [MSLscheduleLength,MSLScheduleResult] = MSLFuc( DAG_backup, relReq, failureRate_backup);
% totalEnergyConsumption_backup = EnergyConsumptionFreqFuc(MSLScheduleResult, MSLscheduleLength, energySpec, frequencyScale); %
for k = 1:numProcessor
    eval(strcat('ScheduleResultProi.processor',num2str(k),'=[];'));
end
ASFlag = [0,0]; % 加减的标志

ProscheduleLength = MSLscheduleLength;
if MSLscheduleLength == 0
    totalEnergyConsumption_backup = 0;
    % fprintf(1,'无法满足可靠性而无法调度\n\n');
elseif MSLscheduleLength > DAG_backup.relativeDeadline
    totalEnergyConsumption_backup = 0;
    % fprintf(1,'DVFS函数中20行左右无法满足截至时间\n\n');
else
    % 进行第一步，整体缩放
    fx1 = MSLscheduleLength / DAG_backup.relativeDeadline; % 以下四行是取大的两位小数
    fx2 = fx1 * 100;
    fx2 = ceil(fx2);
    fx3 = fx2 / 100;
    fx_Choiced = fx3;
    ...
    for numPro = 1:numProcessor 
        ...
        while 1
            ...
            if fx_Choicedi < energySpec(numPro,5) % 小于最小频率时要让其选择最小频率
                ...
                ProscheduleLength = AFTForiTaskProi;
                break
            end
